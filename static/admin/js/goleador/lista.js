(() => {
    const form = document.getElementById('goleadores-filter-form');
    const results = document.getElementById('goleadores-results');
    ...
    const renderResults = async (href) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error('Error al cargar los goleadores');
            ...
            temp.innerHTML = html;
            const inner = temp.querySelector('#goleadores-results');
